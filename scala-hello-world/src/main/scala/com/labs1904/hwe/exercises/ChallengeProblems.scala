 ...
 */
 def containsCar(list:List[String]):List[String] = {
  ...
  def sumList(list:List[Int]):Int = {
    //also could use recursion or sum
    val sum = list.foldLeft(0)((accumulator, valueToBeAdded) => accumulator + valueToBeAdded)
    sum
  }
  ...
  10. Same question as #9, but this time you are given a Option[Int]
    ...
    -Returns - Option[Int]
 */
  def catsAgeOption(catsAge: Option[Int]):Option[Int]={
    val humanAge = catsAge.map(x => x*4)
    humanAge
  }
 ...
 */
  def minimumOption(list:List[Option[Int]]): Option[Int] = {
     val min = list.minBy(x => x.getOrElse(0))
    min
  }
